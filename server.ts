...
function isValidApiKey(key: string | undefined): boolean {
  ...
  app.post("/api/gemini/pricing-suggest", async (req, res) => {
    ...
    try {
      ...
      console.error("Gemini API Error in pricing-suggest:", error);
      res.json(getSimulatedPricingSuggestion(proposal, config));
    }
  });
...
- **Competitividade**: O orçamento atual apresenta competitividade **EXCELENTE**. A margem de contribuição operacional é saudável e o preço final está em conformidade com as tabelas de referência do CREA e associações de engenharia mecânica para serviços de complexidade **${proposal.complexity.toUpperCase()}**.`;
  ...
  app.post("/api/gemini/proposal-generate", async (req, res) => {
    ...
    try {
      ...
      console.error("Gemini API Error in proposal-generate:", error);
      res.json(getSimulatedProposalGenerate(services, clientInfo, pricingInfo));
    }
  });
  ...
  app.post("/api/gemini/hvac-load-audit", async (req, res) => {
    try {
      ...
      console.error("Gemini API Error for HVAC Load Audit:", error);
  ...
  app.post("/api/gemini/montacargas-audit", async (req, res) => {
    try {
      ...
      if (!isValidApiKey(apiKey)) {
        ...
        return res.json({
          checklist: {},
          hrn_before: req.body.hrn_before || { lo: 2, fe: 2, dph: 2, np: 2, val: 16 },
          hrn_after: req.body.hrn_after || { lo: 0.033, fe: 1, dph: 1, np: 1, val: 0.033 },
          nao_conformidades: [
          ...
          conclusao: { status: "Aprovado com Restrições", parecer: "O equipamento Monta-Cargas encontra-se operacional, porém necessita de adequações urgentes de segurança física para operação segura." },
          ...
          secoes: {
            introducao: "Este laudo pericial documenta as condições operacionais e de conformidade do monta-cargas.",
      ...
      console.error("Gemini API Error for Montacargas Audit:", error);
  ...
  app.post("/api/gemini/nr13-audit", async (req, res) => {
    ...
    try {
      ...
      console.error("NR-13 endpoint error:", err);
      res.status(500).json({ error: "Falha interna no auditor NR-13: " + err.message });
    }
  });
