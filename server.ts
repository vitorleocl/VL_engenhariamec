...
- **Competitividade**: O orçamento atual apresenta competitividade **EXCELENTE**. A margem de contribuição operacional é saudável e o preço final está em conformidade com as tabelas de referência do CREA e associações de engenharia mecânica para serviços de complexidade **${proposal.complexity.toUpperCase()}**.`;
  ...
  // API: Gerador de Laudos IA de Segurança Contra Incêndio (PPCI, AVCB, CLCB)
  app.post("/api/gemini/fire-safety-audit", async (req, res) => {
    const {
      laudoType = 'avcb',
      state = 'PE',
      clientName,
    ...
    try {
      ...
      const systemPrompt = `
IDENTIDADE
Você é o "Gerador de Laudos IA" da VL ENGENHARIA, um assistente especializado em produzir laudos técnicos de Segurança Contra Incêndio (PPCI, AVCB e CLCB) a partir de dados estruturados de vistoria/levantamento, fornecidos pelo usuário ou importados dos checklists de campo.

TIPOS DE LAUDO SUPORTADOS
1. LAUDO DE PRÉ-PROJETO PPCI — diagnóstico técnico que embasa o dimensionamento do projeto a ser elaborado (ainda não há projeto aprovado).
2. LAUDO DE VISTORIA PARA AVCB — verificação de conformidade de edificação com projeto já aprovado, para fins de emissão/renovação do Auto de Vistoria.
3. LAUDO DE VISTORIA PARA CLCB — equivalente ao AVCB nos estados que adotam esta nomenclatura (adaptar terminologia e base normativa ao estado informado).

FLUXO E REGRAS DE CONTEÚDO
- Tipo de Laudo Solicitado: ${laudoType.toUpperCase()}
- Estado da Edificação: ${state} (Aplica a Instrução Técnica / Norma Técnica do Corpo de Bombeiros de ${state})
- Nunca inventar normas, números de processo, medidas ou resultados de ensaio. Se o dado não foi informado, indicar [A PREENCHER].
- Toda citação normativa deve indicar o número da NBR ou da IT/NT do Corpo de Bombeiros de ${state} e, quando aplicável, o item específico.
- Manter separação clara entre FATO OBSERVADO e PARECER TÉCNICO.
- No laudo de AVCB/CLCB, todo item "não conforme" deve indicar: norma violada, risco associado e ação corretiva recomendada com prazo.
- Não emitir conclusão de aprovação/reprovação em nome do Corpo de Bombeiros — o laudo apresenta PARECER TÉCNICO do responsável técnico, sujeito à vistoria oficial do Corpo de Bombeiros.
- Gerar obrigatoriamente um "Resumo para o Cliente" em linguagem simples (não técnica), de no máximo 10 linhas, explicando a situação e os próximos passos.

Retorne estritamente um JSON estruturado no seguinte formato:
{
  "diagnosticoTecnico": "Análise descritiva detalhada da edificação, situação dos sistemas e riscos observados...",
  "parecerTecnico": "Parecer conclusivo do Engenheiro Responsável Técnico...",
  "parecerFinal": "APTO" | "APTO_COM_PENDENCIAS" | "NAO_APTO",
  "justificativaParecer": "Justificativa detalhada do parecer final...",
  "resumoCliente": "Resumo executivo de até 10 linhas em linguagem clara e acessível ao cliente final...",
  "resumoExecutivo": {
    "totalItens": 10,
    "conformes": 8,
    "naoConformes": 2,
  ...
  "pendenciasIdentificadas": [
    {
      "num": 1,
      "descricao": "Descrição da não conformidade observada",
  ...
  "normasAplicaveis": [
    ...
    "IT/NT do Corpo de Bombeiros do estado"
  ],
  "recomendacoes": [
...
DADOS ENVIADOS PARA A GERAÇÃO DO LAUDO DE SEGURANÇA CONTRA INCÊNDIO:
- Tipo de Laudo: ${laudoType.toUpperCase()}
- Número do Laudo: ${laudoNumber || 'A PREENCHER'}
- Edificação / Cliente: ${clientName || 'A PREENCHER'} (CNPJ/CPF: ${cnpj || 'A PREENCHER'})
- Endereço: ${address || 'A PREENCHER'}, ${city || 'Recife'} - ${state}
- Processo Corpo de Bombeiros nº: ${processoCB || 'A PREENCHER'}
- Projeto Aprovado nº / data: ${projetoAprovado || 'A PREENCHER'}
- Responsável Técnico: ${responsavelTecnico || 'Eng. Vitor Leonardo'} (${registroCrea || 'CREA-PE 048192-D'}, ART/RRT: ${artRrt || 'A PREENCHER'})
- Data da Vistoria: ${laudoDate || 'Data Atual'}
- Área Construída: ${areaConstruida || 'A PREENCHER'} m²
- Número de Pavimentos: ${numeroPavimentos || 'A PREENCHER'}
- Altura da Edificação: ${alturaEdificacao || 'A PREENCHER'} m
- Ocupação / Uso: ${tipoOcupacao || 'A PREENCHER'}
- Carga de Incêndio: ${cargaIncendio || 'A PREENCHER'}
- População Estimada: ${populacaoEstimada || 'A PREENCHER'}
- Fonte de Água / RTI: ${fonteAgua || 'A PREENCHER'} / Reserva: ${volumeReservaAgua || 'A PREENCHER'}
- Notas da Vistoria / Observações do Técnico: ${notes || 'Nenhuma observação adicional.'}

CHECKLIST DE CAMPO PREENCHIDO:
${JSON.stringify(checklist, null, 2)}
`;
      ...
      if (isValidApiKey(apiKey)) {
        ...
        try {
          ...
          console.warn("Failed to parse Gemini output as JSON for Fire Safety Audit:", responseText);
        }
      }

      return res.json(getSimulatedFireSafetyAudit({
        laudoType,
        state,
        clientName,
        processoCB,
      ...
      logGeminiError("Fire Safety Audit", err);
      res.json(getSimulatedFireSafetyAudit({
        laudoType,
        state,
        clientName,
        processoCB,
  ...
  function getSimulatedFireSafetyAudit(params: any) {
    const state = params.state || 'PE';
    const totalItens = (params.checklist && params.checklist.length) || 10;
    
    let conformes = 0;
    let naoConformes = 0;
    if (params.checklist) {
      params.checklist.forEach((item: any) => {
        if (item.status === 'CONFORME' || item.possui === 'SIM') conformes++;
        else if (item.status === 'NAO_CONFORME' || item.possui === 'NAO') naoConformes++;
      });
    } else {
      conformes = 8;
      naoConformes = 2;
    ...
    const pct = Math.round((conformes / (totalItens || 1)) * 100);

    return {
      diagnosticoTecnico: `Análise pericial da edificação de ${params.clientName || 'Contratante'}. Foi realizado o levantamento de segurança contra incêndio e pânico considerando as Instruções Técnicas do Corpo de Bombeiros do estado de ${state} e as normas ABNT NBR vigentes. A edificação apresenta sistemas instalados que necessitam de pequenos ajustes de manutenção operacional e calibração para plena liberação do certificado de conformidade.`,
      parecerTecnico: `Com base nas verificações efetuadas na vistoria e na análise da documentação técnica disponibilizada, a edificação encontra-se parcialmente adequada. ${naoConformes > 0 ? 'Existem pendências técnicas pontuais identificadas que devem ser sanadas dentro do prazo recomendado.' : 'Todas as exigências normativas aplicáveis estão plenamente atendidas.'}`,
      parecerFinal: naoConformes > 0 ? "APTO_COM_PENDENCIAS" : "APTO",
      justificativaParecer: `Edificação atende aos requisitos estruturais fundamentais, dependendo da regularização das ${naoConformes} pendências operacionais indicadas para homologação conclusiva.`,
      resumoCliente: `Realizamos a vistoria técnica de segurança contra incêndio do seu imóvel. A estrutura geral está em boas condições de segurança. Identificamos apenas ${naoConformes} pendência(s) simples que precisam de atenção (como ajuste na mola da porta corta-fogo e reposição da chave do hidrante). Após a correção dessas pendências, o imóvel estará pronto para a obtenção do seu certificado do Corpo de Bombeiros.`,
      resumoExecutivo: {
        totalItens,
        conformes,
        naoConformes,
        pctConforme: pct,
        pendenciasCriticas: naoConformes > 1 ? 1 : 0,
        pendenciasLeves: Math.max(0, naoConformes - 1)
      },
      pendenciasIdentificadas: [
        {
          num: 1,
          descricao: "Abrigo de hidrante nº 02 sem chave de mangueira e acoplamento Storz com indício de oxidação superficial.",
          normaInfringida: `NBR 13714 / IT do Corpo de Bombeiros de ${state}`,
          risco: "MÉDIO",
          prazoSugerido: "15 dias"
        }
      ],
      normasAplicaveis: [
        ...
        `Instruções Técnicas e Normas do Corpo de Bombeiros Militares de ${state}`
      ],
      recomendacoes: [
        "Efetuar lubrificação preventiva e desoxidação dos acoplamentos Storz de todos os hidrantes.",
        "Verificar mensalmente a pressão dos manômetros e as validades dos extintores portáteis.",
        "Manter rotas de fuga e corredores desobstruídos permanentemente."
      ]
    };
  }
