...
export default function LaudoNR12Indep({ onBack, initialPrefilled = false, clients }: { onBack?: () => void, initialPrefilled?: boolean, clients?: ClientData[] }) {
  ...
  const [sectionsConfig, setSectionsConfig] = useState<SectionConfig[]>([
    { id: "secao1", label: "Introdução, Escopo e Objetivos do Laudo", visible: true },
    { id: "secao2", label: "Identificação do Estabelecimento Contratante", visible: true },
    { id: "secao3", label: "Qualificação Técnica da Empresa Contratada (VL Engenharia)", visible: true },
    { id: "secao4", label: "Dados Técnicos de Fabricação do Equipamento", visible: true },
    { id: "secao5", label: "Documentos Técnicos Analisados na Perícia", visible: true },
    { id: "secao6", label: "Normas Regulamentadoras e Legislações Aplicáveis", visible: true },
    { id: "secao7", label: "Metodologia Quantitativa Hazard Rating Number (HRN)", visible: true },
    { id: "secao8", label: "Relatório de Inspeção Visual e Fotográfica", visible: true },
    { id: "secao9", label: "Checklist de Conformidade com a NR-12", visible: true },
    { id: "secao11", label: "Categorização dos Sistemas de Comando (NBR 14153)", visible: true },
    { id: "secao12", label: "Apreciação de Riscos — Classificação HRN", visible: true },
    { id: "secao13", label: "Laudo de Não Conformidades Regulamentares", visible: true },
    { id: "secao15", label: "Cronograma de Plano de Ação Recomendado", visible: true },
    { id: "secao16", label: "Conclusão Pericial e Parecer Conclusivo", visible: true },
    { id: "secao17", label: "Limitações e Observações de Campo", visible: true },
  ]);

  const orderedNumberedSections = getNumberedSections(sectionsConfig);
  const sectionNumbers: Record<string, number | null> = {};
  orderedNumberedSections.forEach((s) => {
    sectionNumbers[s.id] = s.computedNumber;
  });
    ...
    <div className={
      ...
      {activeTab === "preview" && (
        ...
        <div className="p-6 md:p-8 bg-slate-100 dark:bg-slate-950 flex-grow text-slate-950">
          ...
          {/* CONFIGURAÇÃO E GERENCIADOR DE SEÇÕES DO RELATÓRIO */}
          <div className="max-w-5xl mx-auto mb-6 print:hidden">
            <SectionOrderToolbar
              ...
              onApplyAIDiagnostic={(diag) => {
                if (diag.conclusion) {
                  setConclusaoParecer(diag.conclusion);
                }
                if (diag.summary) {
                  setLaudoParams(prev => ({
                    ...prev,
                    notes: `${prev.notes}\n\n[Diagnóstico IA]: ${diag.summary}`
                  }));
                }
              }}
            />
          ...
          <div id="laudo-nr12-printable-area" className="max-w-4xl mx-auto bg-white border border-slate-200 shadow-2xl p-8 md:p-14 text-left leading-relaxed text-slate-900 rounded-3xl print:border-none print:shadow-none print:p-0 print:rounded-none">
            ...
            {printConfig.sumario && (
              <div className="py-12 border-b space-y-4" style={{ pageBreakAfter: "always" }}>
                ...
                <div className="space-y-2 font-mono text-[11px] text-slate-600 font-medium">
                  {orderedNumberedSections.filter(s => s.visible).map((s) => (
                    <div key={s.id} className="flex justify-between border-b border-dashed border-slate-200 pb-1">
                      <span>{s.computedNumber ? `SEÇÃO ${s.computedNumber}:` : ''} {s.label}</span>
            ...
            {/* SEÇÕES PERSONALIZADAS ADICIONADAS PELO USUÁRIO */}
            {orderedNumberedSections.filter(s => s.isCustom && s.visible).map(s => (
              <CustomSectionRenderer
                key={s.id}
                customSection={s.customData!}
                sectionNumber={s.computedNumber}
                reportTitle={`LAUDO TÉCNICO NR-12 — ${laudoParams.equipmentName}`}
              />
            ))}
