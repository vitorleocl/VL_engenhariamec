...
import { Plus, Edit2, Trash2, Search, X, Cog, Save, Tag, FileText } from 'lucide-react';

const EQUIPMENT_TYPE_PRESETS = [
  'Veículo / Automóvel / Motocicleta',
  'Caminhão tipo Munck / Guindaste Articulado',
  'Guindaste Telescópico / Rodoviário',
  'Compressor de Ar / Vaso de Pressão (NR-13)',
  'Caldeira a Vapor / Tubulação (NR-13)',
  'Prensa Hidráulica / Prensa Mecânica (NR-12)',
  'Máquina Pesada / Escavadeira / Pá Carregadeira',
  'Elevador de Carga / Monta-Cargas / Plataforma',
  'Central de Ar Condicionado / Chiller / PMOC (HVAC)',
  'Brinquedo Infantil / Playground / Parque',
  'Estrutura Metálica / Cobertura Industrial',
  'Ponte Rolante / Pórtico / Talha Elétrica',
  'Plataforma Elevatória Móvel (PEMT)',
  'Gerador de Energia / Grupo Gerador Diesel',
  'Silo / Reservatório / Tanque',
  'Exaustor Industrial / Sistema de Exaustão',
...
export default function EquipmentManager({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    const saveObj: EquipmentData = {
      ...
      idTag: currentEq.idTag || '',
      description: currentEq.description || ''
    };
  ...
  const filtered = equipments.filter(e => 
    ...
    (e.idTag && e.idTag.toLowerCase().includes(searchQuery.toLowerCase())) ||
  ...
  const handleOpenEditModal = (eq: EquipmentData) => {
    setError(null);
    setCurrentEq(eq);
    const inPreset = EQUIPMENT_TYPE_PRESETS.includes(eq.type);
    setIsCustomType(!inPreset);
    setModalOpen(true);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">Gerencie máquinas pesadas, veículos, parquinhos, guindastes, compressores e centrais de HVAC</p>
        </div>
      ...
      <div className="relative max-w-md bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
        ...
        <input
          type="text"
          placeholder="Filtrar por tipo, TAG, marca, modelo, cliente ou número de série..."
          value={searchQuery}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 text-xs font-bold font-mono text-slate-400 uppercase tracking-widest">
                  <th className="p-4">Equipamento Ativo & TAG</th>
                  <th className="p-4">Marca & Modelo</th>
                  <th className="p-4">Proprietário / Cliente</th>
                  <th className="p-4">Nº Série / Fabricação</th>
                  ...
                  <tr key={eq.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/35 transition-colors">
                    <td className="p-4">
                      <div className="space-y-1">
                        ...
                        {eq.idTag && (
                          <div className="flex items-center gap-1 text-[11px] font-mono text-amber-700 dark:text-amber-400 font-bold bg-amber-500/10 border border-amber-500/20 px-2 py-0.5 rounded w-fit">
                            <Tag className="w-3 h-3 text-amber-500" />
                            <span>TAG: {eq.idTag}</span>
                          </div>
                        )}
                        {eq.description && (
      ...
      {modalOpen && currentEq && (
        <div className="fixed inset-0 z-50 bg-slate-950/50 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-y-auto relative my-8">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              {/* ID Objeto / TAG & Tipo do Equipamento */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 uppercase font-mono flex items-center gap-1">
                    <Tag className="w-3.5 h-3.5 text-amber-500" />
                    <span>ID Objeto / TAG</span>
                  </label>
                  <input
                    type="text"
                    value={currentEq.idTag || ''}
                    onChange={(e) => setCurrentEq({ ...currentEq, idTag: e.target.value })}
                    ...
                    placeholder="Ex: TAG-102, CHASSI-01, OBJ-99"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <select
                    value={isCustomType ? 'Outro Tipo (Especificar)' : (currentEq.type || EQUIPMENT_TYPE_PRESETS[0])}
                    onChange={(e) => {
                      const val = e.target.value;
                      if (val === 'Outro Tipo (Especificar)') {
                        setIsCustomType(true);
                        setCurrentEq({ ...currentEq, type: '' });
                      } else {
                        setIsCustomType(false);
                        setCurrentEq({ ...currentEq, type: val });
                      }
                    }}
                    className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none text-slate-950 dark:text-white cursor-pointer"
                  >
                    {EQUIPMENT_TYPE_PRESETS.map(typeOpt => (
              ...
              {/* Custom type specify input if custom option chosen */}
              {isCustomType && (
                <div className="space-y-1 animate-fade-in">
                  <label className="text-xs font-bold text-amber-600 dark:text-amber-400 uppercase font-mono">Especificar Tipo Personalizado *</label>
                  <input
                    type="text"
                    required
                    value={currentEq.type || ''}
                    onChange={(e) => setCurrentEq({ ...currentEq, type: e.target.value })}
                    className="w-full bg-amber-500/5 border border-amber-500/30 rounded-lg px-3 py-2 text-sm outline-none text-slate-950 dark:text-white"
                    placeholder="Digite a denominação do ativo (Ex: Picador Florestal)"
                  />
                </div>
              )}
              ...
              <div className="space-y-1">
                <label className="text-xs font-bold text-slate-400 uppercase font-mono flex items-center gap-1">
                  ...
                  <span>Descrição Breve / Observações (Opcional)</span>
                </label>
                <textarea
                  rows={2}
                  ...
                  className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm outline-none text-slate-950 dark:text-white"
                  placeholder="Ex: Localizado no Galpão 2, setor de estamparia. Utilizado em regime contínuo."
                />
              </div>
