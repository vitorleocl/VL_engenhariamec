...
import { Shield, Cpu, Sparkles, Wand2, Truck, FileText, ArrowRight, Car, Layers, Wrench, Activity, BarChart3, Calculator, Hammer, Anchor, Trash2, Search, Clock, ChevronDown, ChevronUp, CheckCircle, Info } from 'lucide-react';
...
import { getGeneratorLaudos, deleteGeneratorLaudo, SavedGeneratorLaudo, getLocalList } from '../../lib/generatorStorage';
...
export default function LaudoGenerators({ clients }: LaudoGeneratorsProps = {}) {
  const [selected, setSelected] = useState<'none' | 'nr12' | 'nr13' | 'heavy' | 'munck' | 'guindaste' | 'vehicle' | 'montacargas' | 'playground' | 'pmoc' | 'art_manutencao' | 'pcm' | 'hvac_carga_termica' | 'school_bus' | 'sinistro_veicular'>('none');
  ...
  useEffect(() => {
    ...
    const fetchSaved = async () => {
      try {
        const list = await getGeneratorLaudos();
        if (isMounted) {
          setSavedLaudos(list);
        }
      } catch (err) {
  ...
  }, [selected]); // Refetch when returning from editor!
  ...
  if (selected === 'sinistro_veicular') {
    return <LaudoAvaliacaoSinistroVeicularIndep 
      onBack={() => { setSelected('none'); setPrefilled(false); setSelectedSavedId(null); setSelectedSavedData(null); }} 
      initialPrefilled={prefilled} 
      clients={clients} 
      initialData={selectedSavedData}
      initialSavedId={selectedSavedId}
    />;
  }
            ...
            <div className="py-12 text-center space-y-4 max-w-md mx-auto">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[11px] text-slate-500 leading-relaxed font-sans">
                  Você ainda não possui nenhum laudo técnico arquivado. Preencha e salve um laudo (como PMOC ou Sinistro Veicular) para ver seu histórico aqui.
                </p>
              </div>
              <button
                onClick={() => setActiveSubTab('generators')}
                    ...
                    <tr key={laudo.id} className="hover:bg-slate-100/50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="py-3 font-semibold text-slate-800 dark:text-slate-200">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                          laudo.type === 'pmoc'
                            ? 'bg-blue-550/10 text-blue-700 dark:text-blue-350'
                            : 'bg-emerald-550/10 text-emerald-700 dark:text-emerald-350'
                        }`}>
                          {laudo.type === 'pmoc' ? 'PMOC' : 'Sinistro Veicular'}
                        </span>
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {deletingId === laudo.id ? (
                            <div className="flex items-center gap-1.5 animate-pulse">
                              ...
                              <button
                                onClick={() => handleDeleteSaved(laudo.id)}
