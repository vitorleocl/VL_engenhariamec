...
export const DEFAULT_INSTRUCOES_SEGURANCA = `Antes de realizar qualquer atividade de inspeção, limpeza ou manutenção nos sistemas de climatização, devem ser observadas as seguintes medidas de segurança:

Antes de iniciar qualquer procedimento de limpeza ou manutenção, desligue o equipamento e interrompa sua alimentação elétrica por meio do disjuntor ou desconectando-o da tomada, a fim de prevenir riscos de choque elétrico.

Não introduza as mãos, dedos ou quaisquer objetos na grelha de descarga de ar da unidade condensadora (unidade externa), pois o ventilador opera em alta velocidade, podendo causar acidentes graves.

Certifique-se de que a instalação elétrica esteja em conformidade com a ABNT NBR 5410 – Instalações Elétricas de Baixa Tensão, bem como com as demais normas técnicas e regulamentações vigentes.

Verifique se a infraestrutura elétrica do local possui capacidade compatível com a potência dos equipamentos de climatização instalados, incluindo alimentação elétrica, circuitos, proteção e aterramento.

Confirme se os dispositivos de proteção (disjuntores e/ou fusíveis) estão corretamente dimensionados e se o sistema de aterramento encontra-se em perfeitas condições de funcionamento.

Certifique-se de que os condutores elétricos estejam corretamente dimensionados para suportar a corrente nominal dos equipamentos, evitando sobrecargas e aquecimento excessivo.

Durante as atividades de manutenção, tenha cuidado com as bordas metálicas e aletas do trocador de calor da unidade condensadora, pois podem provocar cortes.

Os equipamentos de climatização contêm fluido refrigerante sob pressão e componentes elétricos energizados, representando riscos durante intervenções técnicas. Assim, serviços de instalação, manutenção corretiva, reparos ou recarga de fluido refrigerante deverão ser executados exclusivamente por profissionais qualificados e devidamente capacitados.

Profissionais não especializados poderão executar apenas atividades de manutenção básica, como limpeza externa dos equipamentos, higienização e substituição de filtros, desde que observadas todas as medidas de segurança aplicáveis.

Sempre que aplicável, utilize os Equipamentos de Proteção Individual (EPIs) adequados à atividade, tais como luvas de proteção, óculos de segurança, calçado de segurança e demais equipamentos exigidos pelas normas de Segurança e Saúde no Trabalho.

Antes da energização do equipamento após a manutenção, certifique-se de que todas as tampas, proteções e conexões elétricas estejam corretamente instaladas e que não existam ferramentas ou materiais deixados no interior do equipamento.`;
...
export const PREFILLED_PMOC_PARAMS = {
  ...
  contractNumber: "",
  contractScope: "",
  instrucoesSeguranca: DEFAULT_INSTRUCOES_SEGURANCA,
  descricaoComponentesTexto: `Descrição dos Componentes Principais do Sistema de Climatização:
1. Compressor: Componente responsável por comprimir o fluido refrigerante e fazê-lo circular por todo o circuito frigorífico;
2. Unidade Condensadora (Externa): Responsável por rejeitar o calor absorvido do ambiente interno para o meio externo, transformando o fluido refrigerante de estado gasoso para líquido;
3. Unidade Evaporadora (Interna): Troca calor com o ar do ambiente a ser climatizado, resfriando-o e desumidificando-o através da evaporação do fluido refrigerante;
4. Dispositivo de Expansão (Válvula / Capilar): Promove a queda abrupta de pressão do fluido refrigerante antes de entrar no evaporador, permitindo o processo de resfriamento;
5. Filtros de Ar: Elementos retentores de partículas de poeira e contaminantes, garantindo a qualidade do ar insuflado no ambiente;
6. Ventilador / Rotor / Hélice: Promove o fluxo forçado de ar através dos trocadores de calor;
7. Bandeja e Dreno de Condensado: Coleta a água condensada durante o processo de desumidificação do ar e a direciona com caimento adequado ao ponto de escoamento predial.`,
  componentesImages: [] as Array<{ id: string; url: string; caption: string; description?: string }>,
