...
export default function LaudoAvaliacaoSinistroVeicularIndep({ onBack, initialPrefilled = false, clients, initialData, initialSavedId }: Props) {
  ...
  // AI Evaluation of attached images and parameters via Gemini API
  const handleAIEvaluation = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/gemini/monta-veicular", {
        ...
        body: JSON.stringify({
          laudoNumber: processo.laudoNumber || "LAV-001/2026",
          clientName: proprietario.nome || "Cliente não informado",
          cnpj: proprietario.cpfCnpj || "",
          address: proprietario.endereco || "",
          ownerName: proprietario.nome || "Proprietário",
          brand: veiculo.marca,
          model: veiculo.modelo,
          fabYear: veiculo.anoFabricacao,
          modelYear: veiculo.anoModelo,
          color: veiculo.cor,
          plate: veiculo.placa,
          vin: veiculo.chassi,
          renavam: veiculo.renavam,
          claimType: veiculo.tipoSinistro,
          inspectionCity: processo.localVistoria,
          inspectionDate: processo.dataVistoria,
          notes: historico,
      ...
      if (res.ok) {
        ...
        if (data.checklist) {
          setChecklist(prev => prev.map(item => {
            const match = data.checklist[item.id] || data.checklist[item.name];
            if (match) {
              return { ...item, status: match.resposta || match.status || item.status, notes: match.nota || match.notes || item.notes };
            }
            return item;
          }));
        }
        alert("A Inteligência Artificial da VL Engenharia (Gemini 3.6 Flash) analisou as fotografias do veículo sinistrado e atualizou o laudo com sucesso!");
      } else {
        throw new Error("Resposta inválida do servidor");
      }
    } catch (err) {
      console.warn("Fallback de IA:", err);
      ...
      setHistorico("Trata-se de sinistro por colisão frontal e oblíqua na rodovia. O condutor relata colisão com defesa rígida sob condições climáticas adversas. A colisão ocasionou amassamento severo da porção frontal esquerda, afetando componentes estruturais, para-lama dianteiro esquerdo, capô e suspensão dianteira esquerda.");
      setConclusao("Com base nas avaliações periciais físicas e na Resolução CONTRAN nº 810/2020, o veículo apresenta Danos de MÉDIA MONTA. O veículo é recuperável mediante substituição integral das peças danificadas e alinhamento em mesa monobloco.");
      alert("Laudo processado via motor pericial com sucesso!");
    } finally {
      setLoadingAI(false);
    }
  };
  ...
  const handleExportPDF = async () => {
    ...
    // 1. Clone element to isolate print layout and avoid dark theme/rendering glitches
    const clone = originalElement.cloneNode(true) as HTMLElement;
    clone.id = "perito-sinistro-doc-clone";
    clone.className = "proposal-pdf-export-container bg-white text-slate-900 font-sans text-[11px] leading-relaxed w-[210mm] mx-auto";
    ...
    // Ensure every page block inside clone has dedicated A4 dimensions
    const pageBlocks = Array.from(clone.querySelectorAll(".page-block")) as HTMLElement[];
    pageBlocks.forEach((pb) => {
      pb.classList.add("laudo-page-block");
    ...
    // Convert images to base64 canvas data URLs
    ...
    const opt = {
      margin: 0,
      filename: `Laudo_Sinistro_${veiculo.placa || 'Veiculo'}_${processo.laudoNumber.replace(/\//g, '-')}.pdf`,
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-red-600 hover:bg-red-700 disabled:opacity-60 text-white px-4 py-2 text-xs font-black font-mono uppercase tracking-wider rounded-xl transition-all shadow-md hover:scale-102 cursor-pointer"
            >
              {isGeneratingPDF ? (
                <>
