...
export default function PublicProposalPortal({ proposalId }: { proposalId: string }) {
  ...
  // Refactored isolated PDF export using cloned element and strict A4 page settings
  const handleExportPDF = async () => {
    const originalElement = printAreaRef.current;
    if (!originalElement) return;

    setIsGenerating(true);

    // 1. Clone the element to isolate the print layout and avoid theme/rendering glitches
    const clone = originalElement.cloneNode(true) as HTMLElement;
    clone.id = "proposal-portal-printable-block-clone";
    clone.className = "proposal-pdf-export-container bg-white text-slate-900 font-sans text-xs leading-relaxed w-[210mm] mx-auto";

    // Strip dark class and ensure clear light background on all nodes
    clone.querySelectorAll("*").forEach((node) => {
      const el = node as HTMLElement;
      el.classList.remove("dark");
    });

    // Ensure every page block inside the clone has dedicated A4 dimensions and page breaks
    const pageBlocks = Array.from(clone.querySelectorAll(".page-block")) as HTMLElement[];
    pageBlocks.forEach((pb) => {
      pb.classList.add("proposal-a4-page");
      pb.style.width = "210mm";
      pb.style.height = "297mm";
      pb.style.minHeight = "297mm";
      pb.style.maxHeight = "297mm";
      pb.style.boxSizing = "border-box";
      pb.style.backgroundColor = "#ffffff";
      pb.style.color = "#0f172a";
      pb.style.overflow = "hidden";
      pb.style.pageBreakAfter = "always";
      pb.style.breakAfter = "page";
    });

    // Pre-convert images inside clone to base64 canvas data URLs
    ...
    // Mount clone in a hidden offscreen container
    ...
    try {
      await preprocessStylesheets(clone);
      ...
      if (exporter) {
        await exporter().from(clone).set(opt).save();
      } else {
  ...
  const f_multiplierQty = financials.multiplierQty ?? multiplierQty ?? 1;
  const f_technicalHours = financials.technicalHours ?? financials.estimatedHours ?? technicalHours ?? 16;
  const f_hourlyRate = financials.hourlyRate ?? 220;
  const f_travelKm = financials.travelKm ?? 0;
  const f_lodgingDays = financials.lodgingDays ?? 0;
  const f_artCost = financials.artCost ?? 115.30;
  const f_extraExpenses = financials.extraExpenses ?? 0;
  const f_discountPercent = financials.discountPercent ?? 0;
  const f_taxPercent = financials.taxPercent ?? 16.5;
  const f_profitMargin = financials.profitMargin ?? 30;

  const f_laborCost = f_technicalHours * f_hourlyRate;
  const f_travelCost = f_travelKm * 1.5;
  const f_lodgingCost = f_lodgingDays * 250;
  const f_directCostsSum = f_laborCost + f_travelCost + f_lodgingCost + f_artCost + f_extraExpenses;

  const f_marginPct = f_profitMargin / 100;
  const f_preTaxValue = f_marginPct < 1 ? f_directCostsSum / (1 - f_marginPct) : f_directCostsSum * 1.5;
  const f_profitValue = f_preTaxValue - f_directCostsSum;

  const f_taxesFactor = f_taxPercent / 100;
  const f_totalGeralCalculated = f_taxesFactor < 1 ? f_preTaxValue / (1 - f_taxesFactor) : f_preTaxValue * 1.2;
  const f_impostos = f_totalGeralCalculated - f_preTaxValue;
  ...
  return (
    <div className="space-y-8" id="public-portal">
      ...
      <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl flex flex-wrap items-center justify-between gap-4 shadow-xl print:hidden">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <button 
            ...
            className="flex items-center gap-1.5 bg-[#0B2545] hover:bg-[#134074] disabled:opacity-60 text-white text-xs font-bold font-mono tracking-wider uppercase px-4 py-2.5 rounded-xl transition-all cursor-pointer shadow-md"
          >
            {isGenerating ? (
              <>
      ...
      <div 
        ref={printAreaRef}
        id="proposal-portal-printable-block"
        className="bg-slate-100/50 dark:bg-slate-950/20 text-slate-900 rounded-3xl overflow-hidden p-0 font-sans text-xs leading-relaxed max-w-[210mm] mx-auto space-y-6 print:space-y-0"
      >
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden bg-white">
                <table className="w-full border-collapse text-left text-[10px]">
                  ...
                  <tbody className="divide-y divide-slate-100 text-slate-600">
                    {(proposal.escopoItems && proposal.escopoItems.length > 0 ? proposal.escopoItems : [
