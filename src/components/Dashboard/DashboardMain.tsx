...
export default function DashboardMain() {
  ...
  const handleSignOut = async () => {
    try {
      if (isRealFirebase) {
        await logoutUser().catch(err => console.warn('Firebase logout failed, continuing with local state clearing:', err));
      }
    } catch (err) {
      ...
      // Ensure we are at the acervo hash and force window reload to clear memory and cleanly show the Google sign-in screen
    ...
    <section id="restricted-area" className="py-16 bg-slate-100 dark:bg-slate-950 min-h-screen transition-colors duration-300 scroll-mt-16">
      <div className="max-w-7xl mx-auto px-6 space-y-8">
        ...
        <div className="bg-slate-50 dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[600px]">
          ...
          <div className="dashboard-top-header lg:hidden bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 p-4 space-y-3">
                ...
                <>
                  ...
                  >
                    ...
                    <span className="flex items-center gap-1">
                      <span>Geradores com IA</span>
                      <span className="text-[8px] bg-emerald-500 text-white px-1 rounded font-mono font-black animate-pulse">IA</span>
                    </span>
                  </button>
                  <button
                  ...
                  >
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      <span className="text-[8px] bg-amber-500/10 text-amber-500 px-1 rounded font-mono font-black animate-pulse">NOVO</span>
                    </span>
                  </button>
                  <button
          ...
          <div className="dashboard-sidebar hidden lg:flex lg:col-span-3 bg-white dark:bg-slate-950 border-r border-slate-200 dark:border-slate-800 p-6 flex-col justify-between">
              ...
              <div className="space-y-3 pb-6 border-b border-slate-100 dark:border-slate-800">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="space-y-1 p-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-left">
                    <span className="text-[9px] uppercase font-mono tracking-widest text-slate-400 font-black block leading-none mb-1">Banco de Dados:</span>
                    <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                      <span>Google Firestore (Nuvem)</span>
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-1">
                {role === 'admin' ? (
                  <>
                    ...
                    >
                      <Landmark className="w-4 h-4 shrink-0 text-emerald-500" />
                      <span className="flex items-center gap-1.5">
                        <span>Faturamento</span>
                        <span className="text-[8px] bg-emerald-500 text-white px-1.5 py-0.2 rounded font-mono font-black animate-pulse">NOVO</span>
                      </span>
          ...
          {/* Core Content Area */}
          <div className="dashboard-main-content col-span-1 lg:col-span-9 p-4 md:p-10 text-slate-950 dark:text-white bg-slate-50 dark:bg-slate-900 scrollbar-thin">
            <div className="animate-fade-in max-w-full">
              ...
              {activeTab === 'laudos_indep' && role === 'admin' && (
                <LaudoGenerators clients={clients} equipments={equipments} />
              )}
              ...
              {activeTab === 'billing' && role === 'admin' && (
                <BillingManager clients={clients} />
              )}
              {activeTab === 'portal' && (
                <ClientPortal 
                  clients={clients} 
