...
import { getStandardSectionsForReport, getNumberedSections, SectionConfig } from "../../lib/sectionManager";
import { 
  Shield, 
  FileText, 
  Wand2, 
...
export default function LaudoCaminhaoMunckIndep({ onBack, initialPrefilled = false, clients }: { onBack?: () => void, initialPrefilled?: boolean, clients?: ClientData[] }) {
  ...
  const [sectionsConfig, setSectionsConfig] = useState<SectionConfig[]>(() => getStandardSectionsForReport('munck'));
        ...
        <div className={`lg:col-span-8 space-y-8 ${activeTab === "preview" ? "bg-white text-slate-900 p-8 sm:p-12 border rounded-3xl shadow-sm" : ""}`}>
          ...
          {activeTab === "form" ? (
            <div className="space-y-10">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-xs">
                  ...
                  <div className="space-y-1.5 md:col-span-3 flex items-center gap-2 py-2">
                    <input
                      type="checkbox"
                      id="blankSignatureMunck"
                    ...
                    />
                    <label htmlFor="blankSignatureMunck" className="text-xs font-semibold text-slate-700 dark:text-slate-300 cursor-pointer select-none">
            ...
            <div className="space-y-6">
              ...
              {/* GERENCIADOR E SELEÇÃO DE SEÇÕES + DIAGNÓSTICO IA */}
              <div className="max-w-5xl mx-auto mb-6 print:hidden">
                <SectionOrderToolbar
                  ...
                  onApplyAIDiagnostic={(diag) => {
                    if (diag.conclusion) {
                      setConclusao(prev => ({ ...prev, parecer: diag.conclusion }));
                    }
                    if (diag.summary) {
                      setLaudoParams(prev => ({
                        ...prev,
                  ...
                  <div className="space-y-2 font-mono text-xs leading-relaxed text-slate-600">
                    {orderedNumberedSections.filter(s => s.visible).map((s) => (
                      <div key={s.id} className="flex justify-between border-b border-dashed border-slate-200 pb-1">
                        <span>SEÇÃO {s.computedNumber}: {s.label}</span>
              ...
              {orderedNumberedSections.filter(s => s.isCustom && s.visible).map(s => (
                <CustomSectionRenderer
                  ...
                  reportTitle={`LAUDO TÉCNICO — ${laudoParams.equipmentName}`}
                />
              ))}
