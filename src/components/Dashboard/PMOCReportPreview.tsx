...
import { PMOCChecklistItem, PMOCNaoConformidade, UploadedImage, INITIAL_PMOC_ACTIVITIES, DEFAULT_INSTRUCOES_SEGURANCA } from "./pmocData";
...
interface PMOCReportPreviewProps {
  laudoParams: {
    ...
    descricaoComponentesTexto?: string;
    componentesImages?: Array<{
      id: string;
      url: string;
      caption?: string;
      description?: string;
...
export default function PMOCReportPreview({
  ...
  // Define all possible sections in strict document order
  const ALL_SECTIONS = [
    { id: "secao1", key: "secoes1_2", title: "Introdução e Base Legal" },
    { id: "secao2", key: "secoes1_2", title: "Dados do Estabelecimento" },
    { id: "secao3", key: "secoes3_4_5", title: "Responsável pelo PMOC e Dados do Contrato" },
    { id: "secao4", key: "secoes3_4_5", title: "Inventário dos Sistemas de Climatização" },
    { id: "secao5", key: "secao6", title: "Descrição Detalhada do Sistema" },
    { id: "secaoComponentes", key: "secao6", title: "Descrição dos Componentes do Sistema" },
    { id: "secao6", key: "secao6", title: "Parâmetros Físicos de Operação" },
    { id: "secaoInstrucoes", key: "secao7", title: "Instruções Básicas e Medidas de Segurança" },
    { id: "secao7", key: "secao7", title: "Plano de Manutenção Preventiva por Componente" },
    { id: "secao8", key: "secao8", title: "Cronograma Anual e Diário de Lançamento" },
    { id: "secao9", key: "secoes9_10", title: "Monitoramento de Qualidade de Ar Interior (QAI)" },
    { id: "secao10", key: "secoes9_10", title: "Diretrizes de Operação de Sistemas" },
    { id: "secao11", key: "secao11", title: "Registros e Formulários de Manutenção" },
    { id: "secao12", key: "secao12", title: "Resultados da Inspeção Sanitária e Checklist" },
    { id: "secao13", key: "secao13", title: "Recomendações e Não Conformidades" },
    { id: "secao14", key: "secao14", title: "Conclusão Pericial e Validade Legal" },
    { id: "secao15", key: "secao14", title: "Limitações da Auditoria Técnica" },
    { id: "secao16", key: "secao15", title: "Anexo Fotográfico Técnico de Campo" },
  ];
  ...
  const sectionPageMap: Record<string, string> = {
    ...
    secao4: "secoes3_4_5",
    secao5: "secao6",
    secaoComponentes: "secao6",
    secao6: "secao6",
    secaoInstrucoes: "secao7",
    secao7: "secao7",
    secao8: "secao8",
    secao9: "secoes9_10",
    secao10: "secoes9_10",
  ...
  // Get active sections based on visibility of their page/section keys, preserving natural document order
  const activeSections = ALL_SECTIONS.filter(s => visibleSections[s.key] !== false);

  // Map each active section to its sequential number (1 to N)
  const sectionNumbers: Record<string, number> = {};
  activeSections.forEach((s, idx) => {
    sectionNumbers[s.id] = idx + 1;
  });

  // Pages to keep track of numbering
  const ALL_PAGES = [
    { id: "capa", key: "capa" },
    { id: "apresentacao", key: "apresentacao" },
    { id: "sumario", key: "sumario" },
    { id: "secoes1_2", key: "secoes1_2" },
    { id: "secoes3_4_5", key: "secoes3_4_5" },
    { id: "secao6", key: "secao6" },
    { id: "secao7", key: "secao7" },
    { id: "secao8", key: "secao8" },
    { id: "secoes9_10", key: "secoes9_10" },
    { id: "secao11", key: "secao11" },
    { id: "secao12", key: "secao12" },
    { id: "secao13", key: "secao13" },
    { id: "secao14", key: "secao14" },
    { id: "secao15", key: "secao15" },
  ];

  // Active pages
  const activePages = ALL_PAGES.filter(p => visibleSections[p.key] !== false);
  const totalPages = activePages.length;

  const pageNumbers: Record<string, number> = {};
  activePages.forEach((p, idx) => {
    pageNumbers[p.id] = idx + 1;
  });

  // Safe helper to get page number as formatted string (e.g., "04")
  const getPageNumStr = (pageId: string) => {
    const num = pageNumbers[pageId];
    if (num === undefined) return "--";
    return num < 10 ? `0${num}` : `${num}`;
  };

  const totalPagesStr = totalPages < 10 ? `0${totalPages}` : `${totalPages}`;

  const renderHeader = (title: string, subtitle?: string) => (
    <div className="flex justify-between items-center border-b-2 border-[#134074] pb-2.5 mb-4">
      <div className="flex items-center gap-2.5">
        <Logo className="w-8 h-8 text-[#134074]" />
        <div>
          <span className="font-sans font-black text-xs tracking-wider text-[#134074] leading-none block">VL ENGENHARIA</span>
          <span className="text-[7.5px] uppercase tracking-widest text-slate-500 font-mono block mt-0.5">Consultoria & Laudos Técnicos</span>
        </div>
      </div>
      <div className="text-right font-mono text-[8.5px] text-slate-500">
        <p className="font-bold text-[#134074] uppercase">{title}</p>
        <p>{subtitle || `PMOC Nº ${laudoParams.laudoNumber}`}</p>
      </div>
    </div>
  );

  const renderFooter = (pageKey: string, customText?: string) => (
    <div className="flex justify-between items-center border-t border-slate-200 pt-2 text-[8px] font-mono text-slate-400 mt-auto">
      <span>VL ENGENHARIA | LEI Nº 13.589/2018</span>
      <span className="truncate max-w-[200px]">{laudoParams.clientName}</span>
      <span>{customText || `PÁGINA ${getPageNumStr(pageKey)} DE ${totalPagesStr}`}</span>
    </div>
  );
      ...
      >
        ...
        {visibleSections.apresentacao !== false && (
          ...
          <div className="space-y-6">
            {renderHeader("CARTA DE APRESENTAÇÃO", "PMOC - VL ENGENHARIA")}
          ...
          <div className="flex flex-col items-center justify-center pt-8 border-t border-slate-200">
            <div className="w-64 h-14 border-2 border-dashed border-slate-300 rounded flex flex-col items-center justify-center bg-slate-50 p-1 my-1">
            ...
            <p className="font-bold text-slate-900 mt-1">{laudoParams.rtName}</p>
        ...
        {visibleSections.sumario !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader("SUMÁRIO DE SEÇÕES", "ESTRUTURA DO PMOC")}

            <div className="space-y-3 font-mono text-[10.5px] text-slate-700">
              {activeSections.map((s) => {
                const sPageKey = sectionPageMap[s.id];
                const pageStr = getPageNumStr(sPageKey);
                return (
                  <div key={s.id} className="flex justify-between border-b border-dashed border-slate-200 pb-1">
                    ...
                    <span className="font-bold">Pág. {pageStr}</span>
                  </div>
                );
              })}
            </div>
        ...
        {visibleSections.secoes1_2 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÕES ${sectionNumbers.secao1 || 1} & ${sectionNumbers.secao2 || 2}`, "INTRODUÇÃO, BASE LEGAL E DADOS")}

            {/* SEÇÃO 1 */}
            <div className="space-y-2">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao1 || 1}: INTRODUÇÃO E BASE LEGAL DO PMOC
              </h3>
            ...
            <div className="space-y-2 pt-4">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao2 || 2}: DADOS DO ESTABELECIMENTO DE USO COLETIVO
              </h3>
        ...
        {visibleSections.secoes3_4_5 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÕES ${sectionNumbers.secao3 || 3} & ${sectionNumbers.secao4 || 4}`, "RESPONSÁVEL TÉCNICO E INVENTÁRIO")}
            ...
            <div className="space-y-2">
              <h3 className="font-black text-xs text-emerald-600 dark:text-emerald-500 uppercase font-mono border-l-4 border-emerald-500 pl-2">
                SEÇÃO {sectionNumbers.secao3 || 3}: DADOS DA EMPRESA EMISSORA / RESPONSÁVEL PELO PMOC
              </h3>
              ...
              {/* DADOS DO CONTRATO */}
              {(laudoParams.contractNumber || laudoParams.contractScope) && (
                <div className="mt-2.5 overflow-hidden rounded-xl border border-blue-200 bg-blue-50/20 font-sans p-2.5">
                  <h4 className="font-bold text-[#134074] uppercase text-[9.5px] mb-1">
                    DADOS DO CONTRATO DE PRESTAÇÃO DE SERVIÇOS
                  </h4>
                  <div className="space-y-0.5 text-[10px]">
                    {laudoParams.contractNumber && (
                      <p><strong>Nº do Contrato Vale / Cliente:</strong> <span className="font-mono text-slate-900 font-bold">{laudoParams.contractNumber}</span></p>
                    )}
                    {laudoParams.contractScope && (
                      <p><strong>Escopo:</strong> <span className="text-slate-800">{laudoParams.contractScope}</span></p>
                    )}
                  </div>
                </div>
              )}
            ...
            <div className="space-y-2 pt-4">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao4 || 4}: INVENTÁRIO DOS EQUIPAMENTOS E SISTEMAS DE CLIMATIZAÇÃO
              </h3>
                  ...
                  <tbody className="divide-y divide-slate-150">
                    {appliances.map((ap) => (
                      <tr key={ap.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="p-2 text-slate-800">{ap.marca}</td>
        ...
        {visibleSections.secao6 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÕES ${sectionNumbers.secao5 || 5} & ${sectionNumbers.secao6 || 6}`, "DESCRIÇÃO DO SISTEMA E PARÂMETROS")}

            {/* SEÇÃO 5 */}
            <div className="space-y-2">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao5 || 5}: DESCRIÇÃO DO SISTEMA CLIMATIZADO
              </h3>
              <p className="text-slate-700 leading-relaxed font-sans text-justify">
            ...
            {/* SEÇÃO DESCRIÇÃO DOS COMPONENTES */}
            {laudoParams.descricaoComponentesTexto && (
              <div className="space-y-2 pt-2 break-inside-avoid">
                <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                  SEÇÃO {sectionNumbers.secaoComponentes || 6}: DESCRIÇÃO DOS COMPONENTES DO SISTEMA
                </h3>
                <p className="text-slate-700 leading-relaxed font-sans text-xs text-justify whitespace-pre-line">
                  {laudoParams.descricaoComponentesTexto}
                </p>

                {/* Fotos e Anexos de Componentes */}
                {laudoParams.componentesImages && laudoParams.componentesImages.length > 0 && (
                  <div className="grid grid-cols-2 gap-3 pt-2">
                    {laudoParams.componentesImages.map((img, idx) => (
                      <div key={img.id || idx} className="border border-slate-200 rounded-lg p-2 bg-slate-50/50 flex flex-col items-center text-center break-inside-avoid">
                        <img src={img.url} alt={img.caption || "Componente do Sistema"} className="max-h-36 w-auto object-contain rounded mb-1.5 border border-slate-200" />
                        {img.caption && <p className="font-bold text-[9px] text-[#134074] uppercase">{img.caption}</p>}
                        {img.description && <p className="text-[8px] text-slate-500 mt-0.5 leading-tight">{img.description}</p>}
                      </div>
                    ))}
                  </div>
                )}
            ...
            <div className="space-y-2 pt-4">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao6 || 7}: PARÂMETROS LEGAIS DE OPERAÇÃO E CONFORTO TÉRMICO
              </h3>
              <p className="text-slate-700 font-sans">
        ...
        {visibleSections.secao7 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÃO ${sectionNumbers.secao7 || 7}`, "PLANO DE MANUTENÇÃO PREVENTIVA")}

            {/* INSTRUÇÕES BÁSICAS E SEGURANÇA */}
            <div className="space-y-2 pb-2">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secaoInstrucoes || 8}: INSTRUÇÕES BÁSICAS E MEDIDAS IMPORTANTES DE SEGURANÇA
              </h3>
              <div className="bg-amber-50/40 border border-amber-200/80 rounded-xl p-3 space-y-1 text-slate-800 text-[10px] font-sans leading-relaxed">
                <div className="whitespace-pre-line text-slate-800 text-[10.5px]">
                  {laudoParams.instrucoesSeguranca || DEFAULT_INSTRUCOES_SEGURANCA}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao7 || 9}: PROTOCOLO PADRÃO DE MANUTENÇÃO PREVENTIVA
              </h3>
              <p className="text-slate-700 font-sans">
              ...
              <div className="space-y-4 font-sans text-[10px]">
                ...
                <div className="border border-slate-200 rounded-lg p-3 space-y-1.5 bg-slate-50">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-1">
                    <span className="font-bold text-[#134074] uppercase">{`${sectionNumbers.secao7 || 9}.1`} FILTROS DE AR</span>
                ...
                <div className="border border-slate-200 rounded-lg p-3 space-y-1.5 bg-slate-50">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-1">
                    <span className="font-bold text-[#134074] uppercase">{`${sectionNumbers.secao7 || 9}.2`} SERPENTINAS EVAPORADORAS & BANDEJAS</span>
                ...
                <div className="border border-slate-200 rounded-lg p-3 space-y-1.5 bg-slate-50">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-1">
                    <span className="font-bold text-[#134074] uppercase">{`${sectionNumbers.secao7 || 9}.3`} CONDENSADORAS & COMPRESSOR</span>
                    <span className="font-mono text-[9px] bg-[#134074] text-white px-1.5 py-0.5 rounded font-bold uppercase">Mensal / Semestral</span>
                  </div>
                  <p className="text-slate-650 leading-relaxed text-justify">
                ...
                <div className="border border-slate-200 rounded-lg p-3 space-y-1.5 bg-slate-50">
                  <div className="flex justify-between items-center border-b border-slate-200 pb-1">
                    <span className="font-bold text-[#134074] uppercase">{`${sectionNumbers.secao7 || 9}.4`} SISTEMA ELÉTRICO & FLUIDO REFRIGERANTE</span>
        ...
        {visibleSections.secao8 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÃO ${sectionNumbers.secao8 || 8}`, "CRONOGRAMA ANUAL DE MANUTENÇÃO")}

            <div className="space-y-3">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao8 || 8}: CRONOGRAMA ANUAL E REGISTRO DE MANUTENÇÃO (MÊS A MÊS)
              </h3>
              <p className="text-slate-700 font-sans">
        ...
        {visibleSections.secoes9_10 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÕES ${sectionNumbers.secao9 || 9} & ${sectionNumbers.secao10 || 10}`, "MONITORAMENTO DE QAI E OPERAÇÃO")}

            {/* SEÇÃO 9 */}
            <div className="space-y-2">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao9 || 9}: MONITORAMENTO DA QUALIDADE DO AR INTERIOR (QAI)
              </h3>
              <p className="text-slate-700 leading-relaxed font-sans text-justify">
            ...
            <div className="space-y-2 pt-4">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao10 || 10}: PROCEDIMENTOS DE OPERAÇÃO DOS SISTEMAS
              </h3>
        ...
        {visibleSections.secao11 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÃO ${sectionNumbers.secao11 || 11}`, "FORMULÁRIO DE REGISTRO PREVENTIVO")}

            <div className="space-y-3">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao11 || 11}: FORMULÁRIO DE REGISTRO PREVENTIVO MENSAL (MODELO OFICIAL)
              </h3>
              <p className="text-slate-700 font-sans">
        ...
        {visibleSections.secao12 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÃO ${sectionNumbers.secao12 || 12}`, "CHECKLIST REQUISITOS SANITÁRIOS")}

            <div className="space-y-3">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao12 || 12}: AUDITORIA DE REQUISITOS SANITÁRIOS GERAIS
              </h3>
              <p className="text-slate-700 font-sans">
        ...
        {visibleSections.secao13 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÃO ${sectionNumbers.secao13 || 13}`, "NÃO CONFORMIDADES E PLANO DE AÇÃO")}

            <div className="space-y-3">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao13 || 13}: PLANO DE AÇÃO DE CORREÇÕES TÉCNICAS (CONFORME ISO 12100)
              </h3>
              <p className="text-slate-700 font-sans">
        ...
        {visibleSections.secao14 !== false && (
          <div className="min-h-[255mm] flex flex-col justify-between py-6 page-break-after-always">
          <div className="space-y-6">
            {renderHeader(`SEÇÕES ${sectionNumbers.secao14 || 14} & ${sectionNumbers.secao15 || 15}`, "CONCLUSÃO E PARECER TÉCNICO")}

            {/* SEÇÃO 14 */}
            <div className="space-y-3">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao14 || 14}: PARECER TÉCNICO CONCLUSIVO DE ENGENHARIA
              </h3>
              <p className="text-slate-700 leading-relaxed font-sans text-justify">
            ...
            <div className="space-y-2 pt-4">
              <h3 className="font-black text-xs text-slate-500 uppercase font-mono border-l-4 border-slate-400 pl-2">
                SEÇÃO {sectionNumbers.secao15 || 15}: LIMITAÇÕES E DIRETRIZES DA AUDITORIA
              </h3>
          ...
          <div className="flex flex-col items-center justify-center pt-8 border-t border-slate-200">
            ...
            <p className="font-bold text-slate-900 mt-1">{laudoParams.rtName}</p>
            <p className="text-slate-500 text-[10px] font-mono">Engenheiro Mecânico Responsável | CREA-PE: {laudoParams.rtCrea}</p>
            <p className="text-slate-400 text-[9px] font-mono mb-4">Próxima Revisão Compulsória: {laudoParams.validityDate}</p>
          </div>

          {renderFooter("secao14")}
        </div>
        )}
        ...
        {visibleSections.secao15 !== false && (
          ...
          <div className="space-y-6">
            {renderHeader(`SEÇÃO ${sectionNumbers.secao15 || 15}`, "ANEXO FOTOGRÁFICO DE CAMPO")}

            <div className="space-y-4">
              <h3 className="font-black text-xs text-[#134074] uppercase font-mono border-l-4 border-[#134074] pl-2">
                SEÇÃO {sectionNumbers.secao15 || 15}: EVIDÊNCIAS VISUAIS DE SEGURANÇA E OPERAÇÃO
              </h3>
              <p className="text-slate-700 font-sans">
          ...
          {renderFooter("secao15", `PÁGINA ${getPageNumStr("secao15")} DE ${totalPagesStr} | MEMORIAL TÉCNICO`)}
        </div>
        )}
          ...
          <div key={`sheet-${ap.id}`} className="min-h-[255mm] flex flex-col justify-between py-6 page-break-before-always font-sans text-[10px]">
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-4 gap-2 text-[9px] bg-slate-50 border border-slate-200 p-2 rounded">
                ...
                <div>
                  <p className="text-[7.5px] font-bold text-slate-400 uppercase font-mono">FABRICANTE</p>
                  <p className="font-bold text-slate-900 truncate">{ap.marca}</p>
                </div>
                <div>
              ...
              <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
                <table className="w-full text-left border-collapse text-[8px] font-sans">
                  ...
                  <tbody className="divide-y divide-slate-150 leading-tight">
                    {(() => {
                      const getOrder = (act: any) => {
                        const pStr = (act.periodicidade || "").trim().toLowerCase();
                        const isM = pStr.startsWith("m") || act.col === "M";
                        const isT = pStr.startsWith("t") || act.col === "T";
                        const isS = pStr.startsWith("s") || act.col === "S";
                        if (isM) return 1;
                        if (isT) return 2;
                        if (isS) return 3;
                        return 4;
                      };
                      const sortedActs = [...(ap.atividades || [])].sort((a, b) => getOrder(a) - getOrder(b));
                      
                      if (sortedActs.length > 0) {
                        return sortedActs.map((act, index) => {
                          const pStr = (act.periodicidade || "").trim().toLowerCase();
                          const isMensal = pStr.startsWith("m") || (act as any).col === "M";
                          const isTrimestral = pStr.startsWith("t") || (act as any).col === "T";
                          const isSemestral = pStr.startsWith("s") || (act as any).col === "S";
                          return (
                            <tr key={index} className="hover:bg-slate-50/50">
                              ...
                              <td className="p-0.5 text-center font-black text-[#134074] border-l border-slate-200 bg-slate-50/20 text-[8.5px]">
                                {(isMensal || isTrimestral) ? "X" : ""}
                              </td>
                              <td className="p-0.5 text-center font-black text-blue-600 border-l border-slate-200 bg-slate-50/20 text-[8.5px]">
                                {(isMensal || isTrimestral || isSemestral) ? "X" : ""}
                              </td>
                            </tr>
                          );
                        });
                      } else {
                        const sortedTemplate = [...DETAILED_ACTIVITIES_TEMPLATE].sort((a, b) => {
                          const getTmplOrder = (x: any) => x.col === "M" ? 1 : (x.col === "T" ? 2 : 3);
                          return getTmplOrder(a) - getTmplOrder(b);
                        });
                        return sortedTemplate.map((act, index) => (
                          <tr key={index} className="hover:bg-slate-50/50">
                            <td className="p-0.5 px-1.5 text-slate-800 font-medium">
                              {act.text}
                            </td>
                            ...
                            <td className="p-0.5 text-center font-black text-[#134074] border-l border-slate-200 bg-slate-50/20 text-[8.5px]">
                              {(act.col === "M" || act.col === "T") ? "X" : ""}
                            </td>
                            <td className="p-0.5 text-center font-black text-blue-600 border-l border-slate-200 bg-slate-50/20 text-[8.5px]">
                              {(act.col === "M" || act.col === "T" || act.col === "S") ? "X" : ""}
                            </td>
                          </tr>
                        ));
                      }
