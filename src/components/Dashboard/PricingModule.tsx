...
export default function PricingModule({ clients }: { clients?: ClientData[] } = {}) {
  ...
  const [artCost, setArtCost] = useState<number>(115.30);
  ...
  const calculateFinancials = () => {
    // 1. Base Direct Costs of services
    const servicesBaseSum = selectedServices.reduce((sum, s) => sum + s.basePrice, 0) * multiplierQty;

    // 2. Extra Expenses
    const travelCost = travelKm * 1.5; // R$ 1.50 per Km
    const lodgingCost = lodgingDays * 250; // R$ 250 per day
    const directCostsSum = servicesBaseSum + travelCost + lodgingCost + artCost + extraExpenses;
    ...
    const subtotal = finalCalculated;
    const impostos = subtotal * taxesFactor;
  ...
  const handleEditProposal = (prop: Proposal) => {
    ...
    setArtCost(prop.pricingInfo.artCost ?? 115.30);
  ...
  const handleSaveProposal = async (status: "rascunho" | "enviado") => {
    ...
    try {
      ...
      if (status === "enviado") {
        // Copy secure link to clipboard using the robust mapping helper
        const secureUrl = getSharedUrl(proposalId);
        navigator.clipboard.writeText(secureUrl).then(() => {
          triggerToast("Orçamento publicado! Link seguro de assinatura copiado para a área de transferência.");
  ...
  const deleteProposal = (id: string) => {
    if (confirm("Deseja realmente excluir este orçamento histórico?")) {
                ...
                <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-6 shadow-xl relative overflow-hidden">
                  ...
                  <div className="space-y-4 font-mono text-xs">
                    ...
                    <div className="flex justify-between items-center text-slate-400 border-b border-slate-900 pb-2">
                      ...
                      <span className="text-white">R$ {financials.subtotal.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
                    ...
                    <div className="flex justify-between items-center text-slate-400 border-b border-slate-900 pb-2">
                      <span>Impostos Inclusos ({taxPercent}%):</span>
                      <span className="text-white">R$ {financials.impostos.toLocaleString("pt-BR", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              ...
              <div className="lg:col-span-9 space-y-6">
                ...
                <div 
                  ...
                  className="bg-white text-slate-950 border border-slate-200 rounded-3xl shadow-2xl overflow-hidden p-[12mm] font-sans text-xs leading-relaxed max-w-[210mm] mx-auto text-left"
                >
                    ...
                    <div className="flex flex-col justify-between min-h-[285mm] break-after-page page-block pb-8">
                      ...
                      <div className="my-auto space-y-6 py-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8 border-t border-slate-100">
                          <div className="text-center space-y-2">
                            <p className="font-serif italic text-slate-400 text-[10px]">[Assinado Eletronicamente]</p>
