...
import { exportToWord, copyRichText, preprocessStylesheets, restoreStylesheets, exportToGoogleDocs } from "../../lib/pdfUtils";
...
export default function LaudoPMOCIndep({ onBack, clients, initialData, initialSavedId }: LaudoPMOCIndepProps) {
  // Tabs management
  ...
  React.useEffect(() => {
    if (initialData) {
      ...
      if (initialData.environments) setEnvironments(initialData.environments);
      if (initialData.customActivities) setCustomActivities(initialData.customActivities);
      if (initialData.appliances) setAppliances(initialData.appliances);
      if (initialData.checklist) setChecklist(initialData.checklist);
      if (initialData.naoConformidades) setNaoConformidades(initialData.naoConformidades);
      if (initialData.secoes) setSecoes(initialData.secoes);
      if (initialData.uploadedImages) setUploadedImages(initialData.uploadedImages);
      if (initialData.blankPlanning !== undefined) setBlankPlanning(initialData.blankPlanning);
      if (initialData.visibleSections) setVisibleSections(initialData.visibleSections);
      if (initialData.artPdf) setArtPdf(initialData.artPdf);
    }
  }, [initialData]);

  const handleSave = async () => {
    try {
      const clientName = laudoParams.clientName || "Cliente PMOC";
      const equipmentModel = `${laudoParams.edificioNome || "Edifício"} - PMOC`;
      const date = laudoParams.emissaoData;
      const formData = {
        laudoParams,
        environments,
        customActivities,
      ...
      console.error(err);
      showNotification("error", `Erro ao salvar o laudo de PMOC: ${err?.message || err}`);
    }
  };
  ...
  const [showDocsGuideModal, setShowDocsGuideModal] = useState(false);
  ...
  // Component Images upload & handlers
  const handleComponentImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files) return;
    ...
    Array.from(files).forEach((file: any) => {
      ...
      reader.onloadend = () => {
        ...
        setLaudoParams(prev => {
          const currentList = prev.componentesImages || [];
          return {
            ...prev,
            componentesImages: [
              ...currentList,
              {
                id: `comp_img_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`,
                url: result,
  ...
  const handleUpdateComponentImage = (id: string, field: 'caption' | 'description', val: string) => {
    setLaudoParams(prev => {
      const currentList = prev.componentesImages || [];
      return {
        ...prev,
        componentesImages: currentList.map(img => img.id === id ? { ...img, [field]: val } : img)
      };
    });
  };

  const handleRemoveComponentImage = (id: string) => {
    setLaudoParams(prev => {
      const currentList = prev.componentesImages || [];
      return {
        ...prev,
        componentesImages: currentList.filter(img => img.id !== id)
      };
    });
  };
  ...
  const handleExportGoogleDocs = async () => {
    ...
    showNotification("info", "Formatando e preparando documento para o Google Docs...");
    const success = await copyRichText("pmoc-report-container");
    if (success) {
      setShowDocsGuideModal(true);
      try {
        window.open("https://docs.new", "_blank");
      } catch (err) {
        console.warn("Blocked popup, will prompt user in modal", err);
      }
    } else {
      showNotification("error", "Erro ao preparar laudo para o Google Docs.");
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        ...
        <div className="flex gap-2">
          {onBack && (
            <button 
              onClick={onBack}
              className="px-4 py-2 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-650 text-xs font-bold transition-all cursor-pointer"
            >
              Voltar ao Painel
            </button>
          )}
          ...
          <button
            onClick={handleSave}
            className="flex items-center gap-1.5 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl text-xs font-bold shadow-md shadow-emerald-500/10 transition-all hover:-translate-y-0.5 cursor-pointer"
          >
          ...
          <button
            ...
            disabled={loadingAI}
            className="flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-700 hover:to-indigo-700 disabled:opacity-50 text-white rounded-xl text-xs font-bold shadow-md shadow-indigo-500/10 transition-all hover:-translate-y-0.5 cursor-pointer"
          >
        ...
        {activeTab === "params" && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                <h3 className="font-sans font-black text-sm tracking-tight text-blue-600 dark:text-blue-400 border-b pb-2 uppercase flex items-center gap-2">
                  <FileText className="w-4 h-4" />
                  Dados do Contrato de Serviço (Opcional)
                </h3>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase text-slate-400 font-mono">Nº do Contrato Vale / Cliente</label>
                    <input 
                      type="text" 
                      placeholder="Ex: 4500123456"
                      value={laudoParams.contractNumber || ""}
                      onChange={(e) => handleParamChange("contractNumber", e.target.value)}
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-xs text-slate-800 dark:text-white font-mono"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold uppercase text-slate-400 font-mono">Escopo do Contrato</label>
                    <input 
                      type="text" 
                      placeholder="Ex: Prestação de serviços de manutenção preventiva e corretiva nos condicionadores de ar."
                      value={laudoParams.contractScope || ""}
                      onChange={(e) => handleParamChange("contractScope", e.target.value)}
                      className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-xs text-slate-800 dark:text-white"
                    />
                  </div>
                </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                <div className="flex items-center justify-between border-b pb-2">
                  <h3 className="font-sans font-black text-sm tracking-tight text-amber-600 dark:text-amber-400 uppercase flex items-center gap-2">
                    <Shield className="w-4 h-4" />
                    Instruções Básicas e Medidas de Segurança
                  </h3>
                  <button
                    type="button"
                    onClick={() => handleParamChange("instrucoesSeguranca", DEFAULT_INSTRUCOES_SEGURANCA)}
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase text-slate-400 font-mono">Texto das Medidas Importantes de Segurança (Editável)</label>
                  <textarea 
                    rows={12}
                    value={laudoParams.instrucoesSeguranca ?? DEFAULT_INSTRUCOES_SEGURANCA}
                    onChange={(e) => handleParamChange("instrucoesSeguranca", e.target.value)}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-xs text-slate-800 dark:text-white font-mono leading-relaxed"
                  />
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                <h3 className="font-sans font-black text-sm tracking-tight text-indigo-600 dark:text-indigo-400 border-b pb-2 uppercase flex items-center gap-2">
                  <Cpu className="w-4 h-4" />
                  Descrição dos Componentes do Sistema
                </h3>
                <div className="space-y-1">
                  <label className="text-[10px] font-bold uppercase text-slate-400 font-mono">Texto de Descrição dos Componentes</label>
                  <textarea 
                    rows={6}
                    value={laudoParams.descricaoComponentesTexto || ""}
                    onChange={(e) => handleParamChange("descricaoComponentesTexto", e.target.value)}
                    className="w-full px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-800 dark:bg-slate-950 text-xs text-slate-800 dark:text-white leading-relaxed"
                  />
                </div>
                ...
                <div className="pt-3 border-t border-slate-200 dark:border-slate-800 space-y-3">
                  <div className="flex justify-between items-center">
                    <label className="text-[10px] font-bold uppercase text-slate-400 font-mono">
                      Fotos, Imagens e Anexos dos Componentes
                    </label>
                    <label className="cursor-pointer inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-950/50 hover:bg-indigo-100 text-indigo-700 dark:text-indigo-300 text-xs font-bold rounded-xl border border-indigo-200 dark:border-indigo-800 transition-colors">
                  ...
                  {laudoParams.componentesImages && laudoParams.componentesImages.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {laudoParams.componentesImages.map((img) => (
                        <div key={img.id} className="border border-slate-200 dark:border-slate-800 rounded-xl p-3 bg-slate-50 dark:bg-slate-950 flex gap-3 items-start relative group">
                          <img src={img.url} alt={img.caption} className="w-20 h-20 object-cover rounded-lg border border-slate-200 dark:border-slate-800 shrink-0" />
                          <div className="flex-1 space-y-1.5 min-w-0">
                            <input 
                              type="text" 
                              placeholder="Legenda da foto..."
                              value={img.caption}
                              onChange={(e) => handleUpdateComponentImage(img.id, 'caption', e.target.value)}
                              className="w-full px-2 py-1 text-xs font-bold border border-slate-200 dark:border-slate-800 rounded-lg dark:bg-slate-900 text-slate-800 dark:text-white"
                            />
                            <input 
                              type="text" 
                              placeholder="Descrição adicional (opcional)..."
                              value={img.description || ""}
                              onChange={(e) => handleUpdateComponentImage(img.id, 'description', e.target.value)}
                              className="w-full px-2 py-1 text-[11px] border border-slate-200 dark:border-slate-800 rounded-lg dark:bg-slate-900 text-slate-600 dark:text-slate-400"
                            />
                          </div>
                          <button 
                            type="button"
                            onClick={() => handleRemoveComponentImage(img.id)}
                    ...
                    <p className="text-xs text-slate-400 italic">
                      Nenhuma imagem de componente anexada ainda. Clique no botão acima para anexar fotos (placas técnicas, esquemas, etiquetas ou componentes).
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 rounded-2xl flex flex-wrap gap-2 justify-between items-center shadow-sm">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center gap-1 px-3.5 py-1.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-xs font-bold transition-all shadow-md shadow-blue-500/10 disabled:opacity-50 cursor-pointer"
                >
                  {isExportingDocs ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" /> Preparando Docs...
                    </>
                  ) : (
                    <>
                      <FileText className="w-4 h-4" /> Editar no Google Docs
                    </>
                  )}
                </button>
                <button
      ...
      {showDocsGuideModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-3xl max-w-lg w-full p-6 shadow-2xl relative animate-in fade-in zoom-in duration-200 text-slate-800 dark:text-slate-200">
            <button
              onClick={() => setShowDocsGuideModal(false)}
              className="absolute top-4 right-4 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 p-1.5 rounded-full hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="flex items-center gap-3 mb-5">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">Pronto para o Google Docs!</h3>
                <p className="text-[11px] text-slate-500">O laudo foi copiado com toda a formatação original</p>
              </div>
            </div>

            <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-2xl mb-6 space-y-3.5 border border-slate-150 dark:border-slate-850">
              <p className="text-xs font-semibold text-slate-800 dark:text-slate-200">
                Como os navegadores não permitem colar dados automaticamente em abas externas, siga estes 3 passos simples:
              </p>
              
              <div className="space-y-3">
                <div className="flex gap-3 items-start">
                  <div className="w-5 h-5 shrink-0 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">1</div>
                  <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed">
                    A aba do **Google Docs** foi aberta em branco. Caso ela tenha sido bloqueada ou fechada, use o botão azul abaixo para abrir um novo documento.
                  </p>
                </div>
                
                <div className="flex gap-3 items-start">
                  <div className="w-5 h-5 shrink-0 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">2</div>
                  <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed">
                    Clique em qualquer lugar da **página em branco** do Google Docs para dar foco à página.
                  </p>
                </div>

                <div className="flex gap-3 items-start">
                  <div className="w-5 h-5 shrink-0 bg-blue-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold mt-0.5">3</div>
                  <p className="text-xs text-slate-650 dark:text-slate-400 leading-relaxed">
                    Pressione <kbd className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 rounded font-mono text-[10px] font-semibold text-slate-800 dark:text-slate-100">Ctrl + V</kbd> (Windows) ou <kbd className="px-1.5 py-0.5 bg-slate-200 dark:bg-slate-800 rounded font-mono text-[10px] font-semibold text-slate-800 dark:text-slate-100">Cmd + V</kbd> (Mac). O laudo aparecerá perfeitamente estruturado!
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              <button
                onClick={() => setShowDocsGuideModal(false)}
                className="px-4 py-2 border border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-650 dark:text-slate-300 rounded-xl text-xs font-bold transition-all cursor-pointer"
              >
                Entendi, Fechar
              </button>
              <button
