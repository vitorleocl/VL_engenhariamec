...
export default function BillingManager({ clients = [] }: BillingManagerProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<"todos" | "aprovado" | "enviado" | "rascunho" | "pagos" | "pendentes">("todos");
  ...
  // Load proposals from Firestore and/or LocalStorage with real-time sync
  useEffect(() => {
    setLoading(true);
    ...
    if (isRealFirebase) {
      try {
        unsub = onSnapshot(collection(db, "proposals"), (snapshot) => {
          ...
          console.error("Error listening to proposals in BillingManager:", err);
          if (localProposals.length > 0) setProposals(localProposals);
          setLoading(false);
        });
      } catch (err) {
        console.error("Failed to setup listener for proposals:", err);
        setLoading(false);
      }
    } else {
      setLoading(false);
  ...
  const handleTogglePaid = async (proposalId: string, currentPaid: boolean) => {
    ...
    try {
      ...
      if (updatedProposal && isRealFirebase) {
  ...
  // Handle status update (rascunho / enviado / aprovado)
  const handleUpdateStatus = async (proposalId: string, newStatus: "rascunho" | "enviado" | "aprovado") => {
    setUpdatingId(proposalId);
    try {
      const updatedList = proposals.map(p => {
        if (p.id === proposalId) {
          return { 
            ...p, 
            status: newStatus,
            signedAt: newStatus === 'aprovado' ? (p.signedAt || new Date().toISOString()) : p.signedAt
          };
        }
        return p;
      });
  ...
  const sentProposals = proposals.filter(p => p.status === "enviado");
  const draftProposals = proposals.filter(p => p.status === "rascunho");

  // Financial Metrics
  const totalBilled = approvedProposals.reduce((sum, p) => sum + (p.pricingInfo?.totalGeral || 0), 0);
  const totalSent = sentProposals.reduce((sum, p) => sum + (p.pricingInfo?.totalGeral || 0), 0);
  const totalDrafts = draftProposals.reduce((sum, p) => sum + (p.pricingInfo?.totalGeral || 0), 0);
  const totalOverall = proposals.reduce((sum, p) => sum + (p.pricingInfo?.totalGeral || 0), 0);
  ...
  const filteredProposals = proposals.filter(p => {
    const matchesSearch = 
      ...
      p.associatedLaudoTitle?.toLowerCase().includes(searchTerm.toLowerCase());
    
    if (statusFilter === "todos") return matchesSearch;
    if (statusFilter === "aprovado") return matchesSearch && p.status === "aprovado";
    if (statusFilter === "enviado") return matchesSearch && p.status === "enviado";
    if (statusFilter === "rascunho") return matchesSearch && p.status === "rascunho";
    if (statusFilter === "pagos") return matchesSearch && p.status === "aprovado" && p.paid;
    if (statusFilter === "pendentes") return matchesSearch && p.status === "aprovado" && !p.paid;
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            ...
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm space-y-3 relative overflow-hidden text-left">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Aprovados</span>
                <span className="p-2 bg-emerald-500/10 text-emerald-500 rounded-xl">
                  <DollarSign className="w-5 h-5" />
                </span>
              </div>
              <div>
                <p className="text-[9px] font-mono font-bold text-emerald-500 uppercase">Faturamento Aprovado</p>
            ...
            {/* Card 2: Em Negociação (Enviados) */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm space-y-3 relative overflow-hidden text-left">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Enviados</span>
                <span className="p-2 bg-blue-500/10 text-blue-500 rounded-xl">
                  <ArrowUpRight className="w-5 h-5" />
                </span>
              </div>
              <div>
                <p className="text-[9px] font-mono font-bold text-blue-500 uppercase">Em Negociação</p>
                <h3 className="text-xl font-bold text-blue-600 dark:text-blue-400 truncate">{formatBRL(totalSent)}</h3>
              </div>
              <p className="text-[10px] text-slate-400 font-sans leading-none">
                {sentProposals.length} propostas enviadas aos clientes
              </p>
            </div>

            {/* Card 3: Rascunhos Salvos */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm space-y-3 relative overflow-hidden text-left">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Rascunhos</span>
                <span className="p-2 bg-amber-500/10 text-amber-500 rounded-xl">
                  <Clock className="w-5 h-5" />
                </span>
              </div>
              <div>
                <p className="text-[9px] font-mono font-bold text-amber-500 uppercase">Orçamentos Salvos</p>
                <h3 className="text-xl font-bold text-amber-600 dark:text-amber-400 truncate">{formatBRL(totalDrafts)}</h3>
              </div>
              <p className="text-[10px] text-slate-400 font-sans leading-none">
                {draftProposals.length} orçamentos salvos na base
              </p>
            </div>

            {/* Card 4: Recebido / Quitados */}
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 shadow-sm space-y-3 relative overflow-hidden text-left">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Recebido</span>
                <span className="p-2 bg-purple-500/10 text-purple-500 rounded-xl">
                  <CheckCircle className="w-5 h-5" />
                </span>
              </div>
              <div>
                <p className="text-[9px] font-mono font-bold text-purple-500 uppercase">Total Recebido</p>
                <h3 className="text-xl font-bold text-slate-900 dark:text-white truncate">{formatBRL(totalReceived)}</h3>
              </div>
              <p className="text-[10px] text-slate-400 font-sans leading-none">
                Conversão: {conversionRate.toFixed(1)}% ({totalCreated} elaborados)
              </p>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-white font-sans">Carteira de Orçamentos &amp; Cobranças</h3>
                <p className="text-xs text-slate-500">Acompanhe todos os orçamentos (rascunhos, enviados e aprovados), altere o status e controle os recebimentos.</p>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar cliente, laudo ou código..."
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-sans text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-1 focus:ring-[#134074] w-52"
                  />
                </div>

                {/* Filters */}
                <div className="flex flex-wrap border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden p-0.5 bg-slate-50 dark:bg-slate-950">
                  <button
                    onClick={() => setStatusFilter("todos")}
                    className={`px-2.5 py-1.5 rounded-lg text-[10px] font-mono tracking-wider font-extrabold uppercase transition-all cursor-pointer ${
                      statusFilter === "todos"
                  ...
                  >
                    Rascunhos ({draftProposals.length})
                  </button>
                  <button
            ...
            <div>
              {filteredProposals.length === 0 ? (
                <div className="py-12 text-center text-slate-400 font-mono text-xs">
                  Nenhum orçamento localizado para os filtros selecionados.
                </div>
              ) : (
                <div>
                  {/* MOBILE CARDS VIEW */}
                  <div className="grid grid-cols-1 gap-3 md:hidden">
                    {filteredProposals.map((p) => (
                      <div key={p.id} className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-2xl space-y-3 text-left">
                        <div className="flex items-start justify-between gap-2">
                          <div>
                            <span className="font-mono font-bold text-xs text-slate-900 dark:text-slate-100">{p.id}</span>
                          ...
                          <select
                            ...
                            className={`text-[9px] font-mono font-extrabold uppercase rounded px-2 py-1 border outline-none cursor-pointer ${
                              p.status === "aprovado" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" :
                              p.status === "enviado" ? "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20" :
                        ...
                        {p.associatedLaudoTitle && (
                          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-teal-500/10 text-teal-700 dark:text-teal-300 text-[10px] font-bold rounded-lg border border-teal-500/20">
                            <FileText className="w-3 h-3 text-teal-600" />
                            <span>Laudo IA: {p.associatedLaudoTitle}</span>
                          </div>
                        )}

                        <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-200/50 dark:border-slate-800">
                          <div>
                            <div className="text-[10px] text-slate-400">Valor Total</div>
                            <div className="font-mono font-bold text-slate-900 dark:text-white">{formatBRL(p.pricingInfo?.totalGeral || 0)}</div>
                          </div>
                          <div className="text-right">
                            {p.status === "aprovado" && (
                              <button
                                ...
                                disabled={updatingId === p.id}
                                className={`px-2.5 py-1 rounded-lg font-mono text-[9px] font-black uppercase border cursor-pointer ${
                                  p.paid ? "bg-emerald-500/10 text-emerald-600 border-emerald-500/20" : "bg-amber-500/10 text-amber-600 border-amber-500/20"
                                }`}
                              >
                                {p.paid ? "PACO" : "PENDENTE"}
                              </button>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-end pt-1">
                          <button
                            ...
                            className="px-3 py-1 bg-[#134074] text-white text-[10px] font-bold uppercase rounded-lg cursor-pointer"
                          >
                            Ver Resumo
                          </button>
                        </div>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="border-b border-slate-150 dark:border-slate-800 text-[10px] font-mono tracking-wider text-slate-400 uppercase">
                          ...
                          <th className="py-3 px-4 font-black">Empresa Cliente / Contato</th>
                          <th className="py-3 px-4 font-black">Laudo IA Associado</th>
                          <th className="py-3 px-4 font-black">Status do Orçamento</th>
                          ...
                          <tr key={p.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition-colors">
                            ...
                            <td className="py-3 px-4 font-bold text-slate-800 dark:text-slate-200">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <span className="truncate max-w-[180px] block">{p.clientCompany}</span>
                                  <span className="text-[10px] text-slate-400 font-sans font-normal">{p.clientName}</span>
                                </div>
                              </div>
                            </td>
                            <td className="py-3 px-4 text-slate-500 dark:text-slate-400">
                              {p.associatedLaudoTitle ? (
                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-teal-500/10 text-teal-700 dark:text-teal-300 text-[10px] font-bold rounded-lg border border-teal-500/20 max-w-[180px] truncate" title={p.associatedLaudoTitle}>
                                  <FileText className="w-3 h-3 text-teal-600 shrink-0" />
                                  <span className="truncate">{p.associatedLaudoTitle}</span>
                                </div>
                              ) : (
                                <span className="text-[10px] text-slate-400 font-mono">Sem laudo vinculado</span>
                              )}
                            </td>
                            <td className="py-3 px-4">
                              <select
                                ...
                                onChange={(e) => handleUpdateStatus(p.id, e.target.value as any)}
                                disabled={updatingId === p.id}
                                className={`text-[10px] font-mono font-extrabold uppercase rounded-lg px-2.5 py-1 border outline-none cursor-pointer ${
                                  p.status === "aprovado" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" :
                                  p.status === "enviado" ? "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20" :
                                  "bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20"
                                }`}
                              ...
                              {p.status === "aprovado" ? (
                                <div className="inline-flex items-center justify-center">
                                  ...
                                  >
                                    ...
                                    <span>{p.paid ? "PAGO (QUITADO)" : "PENDENTE"}</span>
                                  </button>
                                </div>
                              ) : (
