...
const initialEquipments: EquipmentData[] = [
  {
    ...
    idTag: 'TAG-PR-01',
    description: 'Prensa hidráulica de corte e dobra no galpão principal'
  },
  {
    id: 'e2',
    clientId: 'c2',
    ...
    idTag: 'MNK-04',
    description: 'Veículo com guindaste articulado para remoção de cargas'
  },
  {
    id: 'e3',
    clientId: 'c3',
    ...
    potenciaInstalada: '',
    idTag: 'PLAY-01',
    description: 'Playground na área de lazer do condomínio'
  },
  {
    id: 'e4',
    clientId: 'c1',
    ...
    idTag: 'CHILLER-01',
    description: 'Unidade de refrigeração para a área administrativa'
  }
];
...
const initialLaudos: LaudoData[] = [
  ...
  },
  {
    ...
    dateInspection: '2025-05-20',
    expirationDate: '2026-05-20',
    rt: 'Vitor Leonardo C. Linhares',
    art: 'PE-18222994-02',
    status: LaudoStatus.VENCIDO,
    pdfUrl: '#',
...
class MockDatabase {
  ...
  getClients(): ClientData[] {
    const list = this.getStorageItem('clients', initialClients);
    ...
    if (cleaned.length !== list.length) {
      this.setStorageItem('clients', cleaned);
    }
    return cleaned;
  }
  ...
  // Laudos
  getLaudos(): LaudoData[] {
    const list = this.getStorageItem('laudos', initialLaudos);
    const cleaned = list.filter(l => 
      !l.clientName?.toLowerCase().includes('teste') && 
      !l.numero?.toLowerCase().includes('teste')
    );
    if (cleaned.length !== list.length) {
      this.setStorageItem('laudos', cleaned);
    }
    return cleaned;
  }
