...
/**
 * Merges the main generated report PDF with an uploaded ART (Anotação de Responsabilidade Técnica) PDF
 * preserving original quality, QR codes, and digital authentication stamps.
 */
export async function mergeReportAndArtPdf(
  mainPdfBytes: Uint8Array | ArrayBuffer, 
  artPdfInput: ArrayBuffer | string
): Promise<Uint8Array> {
  try {
    const mainPdfDoc = await PDFDocument.load(mainPdfBytes);
    
    let artBytes: Uint8Array;
    if (typeof artPdfInput === 'string') {
      const cleanBase64 = artPdfInput.replace(/^data:application\/pdf;base64,/, '');
      const binaryString = atob(cleanBase64);
      artBytes = new Uint8Array(binaryString.length);
      for (let i = 0; i < binaryString.length; i++) {
        artBytes[i] = binaryString.charCodeAt(i);
      }
    } else {
      artBytes = new Uint8Array(artPdfInput);
    }

    const artPdfDoc = await PDFDocument.load(artBytes);
    const copiedPages = await mainPdfDoc.copyPages(artPdfDoc, artPdfDoc.getPageIndices());
    ...
    console.error("Error merging ART PDF with report:", err);
    throw err;
  }
}
  ...
  await Promise.all(
    images.map((img) => {
      return new Promise<void>((resolve) => {
        ...
        // If it's already a base64 raster image (PNG / JPEG), set proportional size constraints and resolve
          ...
          try {
            ...
            if (isLogo) {
              ...
              if (tempImg.naturalWidth > 0 && tempImg.naturalHeight > 0) {
                ...
                h = 100; // Default exact aspect ratio for VL logo (1376x768 -> 180x100)
              }
            } else {
...
export async function exportToWord(elementId: string, filename: string): Promise<void> {
  ...
  // Insert explicit Microsoft Word page breaks between sections/page blocks
  ...
  if (pageBlocks.length > 0) {
    pageBlocks.forEach((block, index) => {
      ...
      if (index > 0) {
        ...
        block.parentNode?.insertBefore(pageBreakP, block);
      }
    });
  } else {
    // Fallback: look for section headers (h1, h2 with "SEÇÃO" or section titles)
    const sectionHeaders = Array.from(clone.querySelectorAll('h1, h2')).filter(h => {
      const txt = (h.textContent || '').toUpperCase();
      return txt.includes('SEÇÃO') || txt.includes('SECAO') || /^\d{1,2}\.\s+[A-Z]/.test(txt.trim());
    });
    sectionHeaders.forEach((header, index) => {
      if (index > 0) {
        ...
        pageBreakP.style.cssText = 'page-break-before: always; break-before: page; mso-break-type: page-break; clear: both; margin: 0; padding: 0; line-height: 0; font-size: 0;';
        pageBreakP.innerHTML = '<br clear="all" style="page-break-before: always; mso-break-type: page-break;" />';
        header.parentNode?.insertBefore(pageBreakP, header);
      }
    });
  }
  ...
  const styles = `
    <style>
      ...
      .laudo-page-block, .page-block { page-break-before: always !important; break-before: page !important; mso-break-type: page-break !important; margin-bottom: 0px !important; padding-bottom: 0px !important; }
      .laudo-page-block:first-child, .page-block:first-child { page-break-before: auto !important; break-before: auto !important; mso-break-type: none !important; }
      ...
      img[src*="logo"], img[alt*="logo"], img[alt*="Logo"], .header-master-logo {
